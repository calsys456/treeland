...
{
    SurfaceWrapper *wrapper = m_rootSurfaceContainer->getSurface(anchor);
    if (!wrapper)
        return false;
    QPoint pos = wrapper->position().toPoint();
    ...
    for (SurfaceWrapper *wrapper : std::as_const(rootSurfaceContainer()->surfaces())) {
        if (wrapper->type() == SurfaceWrapper::Type::XWayland) {
            ...
            qw_xwayland_surface *xwaylandSurface =
                qw_xwayland_surface::try_from_wlr_surface(wlrSurface);
            if (!xwaylandSurface)
                continue;
